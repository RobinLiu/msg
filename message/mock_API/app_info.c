...
//  LOG(INFO, "app group %d", app.group_id);
	for (i = 0; i < MAX_NODE_NUM; ++i)
	{
		...
		{
			if (cluster_info.node_info[i].group_info[j].group_id
					== app.group_id)
			{
				switch (app.role)
				{
				case RANGE_STANDBY:
					if (cluster_info.node_info[i].group_info[j].role
							== RANGE_STANDBY)
					{
						node_list[k++] = cluster_info.node_info[i].node_id;
						LOG(INFO, "Find group %d in node %d",
								app.group_id, node_list[k-1]);
					}
					break;
				case RANGE_ACTIVE:
					if (cluster_info.node_info[i].group_info[j].role
...
void init_node_info(node_info_t* node, int32 index)
{
	...
	if (index > 0 && (index % 2))
	{
		memcpy(node->group_info, (node - 1)->group_info,
				sizeof(node->group_info));
		uint32 k = 0;
...
{
	memcpy(mac, cluster_info.node_info[g_node_id].network_info.mac,
			MAC_ADDR_LEN);
}

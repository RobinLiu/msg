...
bool GetBacktraceStrings(void *const *trace, int size,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (google::Symbolize(static_cast<char *>(trace[i]) - 1,
	  ...
	  //tmp.format("%s [%p]", symbol, trace[i]))
	  
	  snprintf(tmp, arraysize(tmp), "%s [%p]", symbol, trace[i]);
	  std::string tmpstr(tmp);
      trace_strings->push_back(tmpstr);
      symbolized = true;
    } else {
      snprintf(tmp, arraysize(tmp), "%p", trace[i]);
	  std::string tmpstr(tmp);
      trace_strings->push_back(tmpstr);
    }
  }
  ...
  if (trace_symbols.get()) {
    ...
    for (int i = 0; i < size; ++i) {
	  ...
	  ostr<<trace[i];
      trace_strings->push_back(ostr.str());
    }
  }
#endif  // defined(USE_SYMBOLIZE)
